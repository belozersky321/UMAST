        ...
        for (j = 0; j < tree2->nodesNum; j++) //Until all cells in line are filled
        {
            ...
            else //nor a nor w are leaves
            {
                ...
                switch (bestCase){
                    ...
                    default:
                        printf("%d\n", bestCase);
                        fprintf(stderr, "Critical error in main loop, umast:UMASTstep\n");
                        exit(1);
                }
            }
...
void UMAST(Tree* intree1, Tree* intree2)
{
    ...
    sortedSet2 = treeRootAndTopSort(tree2, 1, 0, setPermutation2, 1);
    ...
    for (i = 0; i < rootNum; i++){
        sortedSet1 = treeRootAndTopSort(tree1, rootPositions[i][0], rootPositions[i][1], setPermutation1, 1);
...
{
    printf("Usage:\n");
    printf("%s -i tree1 tree2\n", name);
    printf("\t[-u <1|0>], default 1\n");
    return;
}
...
int main(int argc, char** argv){
    ...
    if (argc < 4)
    {
        ...
        printUmastHelp(argv[0]);
        exit(1);
    }

    param = (char *)malloc(sizeof(char) * 128);
    int startOptionsNum = 1;
    int known, ind1, ind2;
    ...
    while (startOptionsNum < argc)
    {
        ...
        if(param[0] == '-')
        {
            if (strcmp(param, "-i") == 0)
            {
                if (startOptionsNum + 2 < argc)
                {
                    known = 1;
                    ind1 = startOptionsNum + 1;
                    ind2 = startOptionsNum + 2;
                    startOptionsNum += 3;
                }
            }
            if (strcmp(param, "-u") == 0)
            {
                ...
                {
                    known = 1;
                    unrooted = atoi(argv[startOptionsNum + 1]);
                    startOptionsNum += 2;
                }
            }
        }
    ...
    if (unrooted)
    {
        ...
        tree1 = treeRead(argv[ind1], 1);
        tree2 = treeRead(argv[ind2], 1);
    ...
    else
    {
        ...
        fprintf(logfile, "%04d/%02d/%02d %02d:%02d:%02d\n", aTm->tm_year+1900, aTm->tm_mon+1,\
            aTm->tm_mday, aTm->tm_hour, aTm->tm_min, aTm->tm_sec);
        tree1 = treeRead(argv[ind1], 0);
        tree2 = treeRead(argv[ind2], 0);
        if ((tree1->rootId == -1) || (tree2->rootId == -1))
        {
            fprintf(stderr, "Error: trees must be rooted!\n");
            exit(1);
        }
        fprintf(logfile, "Trees read successfully\n");
        fprintf(logfile, "%s\n", treeToString(tree1));
